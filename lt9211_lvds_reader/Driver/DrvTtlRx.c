...
u8 Drv_TtlRx_Pll_Set(void)
{
    ...
    for(ucloopx = 0; ucloopx < 3; ucloopx++)
    {
        ...
        if ((HDMI_ReadI2C_Byte(0x31) & 0x05) == 0x05)
        {
            ...
            LTLog(LOG_INFO,"vmax, vmin = 0x%02x 0x%02x",HDMI_ReadI2C_Byte(0x30) & 0x3f,HDMI_ReadI2C_Byte(0x2f) & 0x3f);
            LTLog(LOG_INFO,"vcoset = 0x%02x",HDMI_ReadI2C_Byte(0x32) & 0x3f); //打印vco_cur自动校准值
            ucrtn = TRUE;
            break;
        }
    }
...
void Drv_TtlRx_SelfTimingSet(void)
{
    ...
    for(ucphase = 0; ucphase < 10; ucphase++)
    {
        ...
        LTLog(LOG_INFO,"phase: 0x%02x", ucphase);
        ...
        LTLog(LOG_INFO,"************timing set phase: 0x%02x", ucphase);
    #endif
    ...
    for (uci = 0; uci < ucResolutionnum; uci++)
    {
        ...
        {
            LTLog(LOG_INFO,"uci fit = 0x%02x",uci);
...
void Drv_TtlRx_VidChkDebug(void)
{
    ...
    u8 ucphase;
        ...
        LTLog(LOG_INFO,"************vedio check phase: 0x%02x", ucphase);
        LTLog(LOG_INFO,"sync_polarity = 0x%02x", ucsync_polarity);
        LTLog(LOG_INFO,"hfp, hs, hbp, hact, htotal = %d %d %d %d %d",ushfp, ushs, ushbp, ushact, ushtotal);
        LTLog(LOG_INFO,"vfp, vs, vbp, vact, vtotal = %d %d %d %d %d",usvfp, usvs, usvbp, usvact, usvtotal);
    #endif
