...
u8 Drv_LvdsRxPll_Cali()
{
    ...
    u32 byteclk_khz;    //byteclk_khz: IO口输入到lvdsrx模块的ref_clk
    ...
    HDMI_WriteI2C_Byte(0x1d,0x2C);  //不加倍，rg_rxpll_doub_clk_en = 0
...
//    #endif
  	...
  	for(ucloopx = 0; ucloopx < 3; ucloopx++)
    {
        ...
        {
            LTLog(LOG_INFO,"_RXPLL_FM_CLK_DET Stable");
            break;
        }
    }
    ...
    LTLog(LOG_INFO,"RXPLL_FM_FREQ_IN_KHZ: %ld",byteclk_khz);
