...
void Mod_VidChkDebug_ForPtn(void)
{
    ...
    LTLog(LOG_INFO,"ucframerate: %d",ucframerate);
    HDMI_WriteI2C_Byte(0xff,0x86);
    ...
    else if ((ushact == pattern_1920x1080_60Hz.hact ) &&(usvact == pattern_1920x1080_60Hz.vact ))
    {
        ...
        else if (ucframerate = (pattern_1920x1080_30Hz.ucFrameRate - 3) && ucframerate < (pattern_1920x1080_30Hz.ucFrameRate + 3))
        {
            LTLog(LOG_INFO,"Video Pattern Set 1920x1080_30Hz");
        }else
        {
...
void Mod_MipiTxpll_ForPtn(struct ChipRxVidTiming *ptn_timing)
{
    ...
    LTLog(LOG_DEBUG, "ulMipiDataRate: %ld, ulMpiTXPhyClk: %ld, ucSericlkDiv: %d, ucPreDiv: %d, ucDivSet: %d",
...
void Mod_MipiTx_PhyTimingParaSet_ForPtn()
{
    ...
    LTLog(LOG_DEBUG, "ck_post (0xD4A9) = 0x%02x", ucClkPost);
    LTLog(LOG_DEBUG, "ck_zero (0xD4A7) = 0x%02x", ucClkZero);
    LTLog(LOG_DEBUG, "hs_lpx  (0xD4A4) = 0x%02x", ucHsLpx);
    LTLog(LOG_DEBUG, "hs_prep (0xD4A5) = 0x%02x", ucHsPrep);
    LTLog(LOG_DEBUG, "hs_trail(0xD4A6) = 0x%02x", ucHsTrail);
    LTLog(LOG_DEBUG, "hs_rqst (0xD48A) = 0x%02x", ucHsRqStPre);
...
void Mod_MipiTxDig_Set_ForPtn()
{
    ...
    LTLog(LOG_DEBUG,"readdly = 0x%02x%02x",(HDMI_ReadI2C_Byte(0x82)), (HDMI_ReadI2C_Byte(0x83)));
...
//        ulLvdsTXPhyClk = ulDessc_Pix_Clk * 3.5;
    ...
    HDMI_WriteI2C_Byte(0x35,ucDivSet);
    LTLog(LOG_DEBUG,"ulPclk_Khz: %ld, ucPreDiv: %d, ucSericlkDiv: %d, ucPixClkDiv: %.1f, ucDivSet: %d",
...
void Mod_TtlTxPhaseSet_ForPtn()
{
    ...
    LTLog(LOG_INFO,"DLL phase set: 0x%02x", ucphase);
...
//    #endif
//    LTLog(LOG_INFO,"idclk DLL best phase: 0x%02x", ucphase);
//
//    HDMI_WriteI2C_Byte(0xff,0x82);
//    HDMI_WriteI2C_Byte(0x65,HDMI_ReadI2C_Byte(0x65) | (ucphase<<4));
//    LTLog(LOG_INFO,"[0x8265]= 0x%02x",HDMI_ReadI2C_Byte(0x65));
//}
//#endif
