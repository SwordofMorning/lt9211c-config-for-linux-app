...
//    HDMI_WriteI2C_Byte(0x4d,0x00); //0x00
    ...
    HDMI_WriteI2C_Byte(0x1e,0x51); //0x51[7:4]pcr diff first step,[3:0]pcr diff second step
    HDMI_WriteI2C_Byte(0x23,0x10); //0x05 MIPIRX PCR capital data set for PCR second step
    HDMI_WriteI2C_Byte(0x24,0x70);
    HDMI_WriteI2C_Byte(0x25,0x80);
    HDMI_WriteI2C_Byte(0x2a,0x10);
...
void Drv_MipiRx_HsSettleSet(void)
{
    ...
    {
        LTLog(LOG_DEBUG, "Set Mipi Rx Settle: 0x%02x", (g_stMipiRxVidTiming_Get.ucLane0SetNum - 5));
        HDMI_WriteI2C_Byte(0xff,0xd0);
...
void Drv_MipiRx_SotGet()
{
    ...
    LTLog(LOG_DEBUG,"Sot Num = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SetNum,g_stMipiRxVidTiming_Get.ucLane1SetNum,
                                                                    g_stMipiRxVidTiming_Get.ucLane2SetNum,g_stMipiRxVidTiming_Get.ucLane3SetNum);
    LTLog(LOG_DEBUG,"Sot Dta = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SotData,g_stMipiRxVidTiming_Get.ucLane1SotData,
...
u8 Drv_MipiRx_VidTiming_Get(void)
{
    ...
    else
    {
        ...
        LTLog(LOG_INFO,"fmt = 0x%02x", g_stMipiRxVidTiming_Get.ucFmt);
        LTLog(LOG_INFO,"pa_lpn = 0x%02x", g_stMipiRxVidTiming_Get.ucPa_Lpn);
        return SUCCESS;
    }
}
...
u8 Drv_MipiRx_VidTiming_Sel()
{
    ...
    for (uci = 0; uci < ucResolutionnum; uci++)
    {
            ...
            ( g_stMipiRxVidTiming_Get.usVact == resolution[uci].usVact ))
        {
            ...
            LTLog(LOG_INFO,"FrameRate = %d", g_stMipiRxVidTiming_Get.ucFrameRate);
               ...
               (g_stMipiRxVidTiming_Get.ucFrameRate < (resolution[uci].ucFrameRate + 3)))
            {
                ...
                LTLog(LOG_INFO,"Video Timing Set %dx%d_%d",g_stRxVidTiming.usHact,g_stRxVidTiming.usVact,g_stMipiRxVidTiming_Get.ucFrameRate);
                rtn = TRUE;
                break;
            }
        }
