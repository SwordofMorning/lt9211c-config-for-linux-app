...
void Mod_ChipID_Read(void)
{
    ...
    LTLog(LOG_INFO,"LT9211C Chip ID: 0x%02x 0x%02x 0x%02x", HDMI_ReadI2C_Byte(0x00),HDMI_ReadI2C_Byte(0x01),HDMI_ReadI2C_Byte(0x02));

    #if LT9211C_VERSION == U1
        LTLog(LOG_INFO,"LT9211C Code Version: U1");
    #elif   LT9211C_VERSION == U2
        LTLog(LOG_INFO,"LT9211C Code Version: U2");
    #endif
}
...
void Mod_SystemTx_SetState(u8 ucState)
{
    if(ucState != g_stChipTx.ucTxState)
    {
        ...
        LTLog(LOG_DEBUG, "TxState = 0x%02x", ucState);
    }
}
...
void Mod_SystemRx_SetState(u8 ucState)
{
    ...
    if(ucState != g_stChipRx.ucRxState)
    {
        ...
        LTLog(LOG_DEBUG, "RxState = 0x%02x", ucState);
