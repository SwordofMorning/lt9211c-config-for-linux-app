...
/*
    MPTX Phy port0/1 mipi driver tap0/1 upside switch impedance select:
    5'b00000 = Hi-Zi;
    5'b00001 = Impedance is 800ohm;
    5'b00010 = Impedance is 400ohm;
    5'b00100 = Impedance is 200ohm;
    5'b01000 = Impedance is 100ohm;
    5'b10000 = Impedance is 50ohm.
    The total impedance is parallel connection of all selected branches.
*/
u8 szTap0Setting[2] = {0x14, 0x14};  //250ohm
u8 szTap1Setting[2] = {0x08, 0x08};  //100ohm
//========================================================================
...
void Drv_MipiTx_DPhySet(void)
{
    ...
    LTLog(LOG_INFO,"MIPI Output PortA");
    ...
    LTLog(LOG_INFO,"MIPI Output PortB");
    #elif (MIPITX_PORT_SEL == DOU_PORT)
    HDMI_WriteI2C_Byte(0x36,(HDMI_ReadI2C_Byte(0x36) | 0x03));
    LTLog(LOG_INFO,"MIPI Output PortA & B");
    #endif
    HDMI_WriteI2C_Byte(0xff,0x82);
...
void Drv_MipiTx_PllSet(IN StructMipiTx* pstMipiTx)
{
    ...
    LTLog(LOG_DEBUG, "ulMipiDataRate:%ld, ulHalfPixClk:%ld, ulMpiTXPhyClk:%ld, ucBpp:%d, ucTxPortNum:0x%02x, ucTxLaneNum:0x%02x",
    ...
    LTLog(LOG_DEBUG,"ucSericlkDiv N1:0x%02x, ucDivSet M2:0x%02x",ucSericlkDiv, ucDivSet);
}
//========================================================================
...
void Drv_MipiTx_PhyTimingParaSet(INOUT StructMipiTx* pstMipiTx, INOUT StructMipiTxDPhy* pstMipiTxDPhy)
{
    ...
    LTLog(LOG_DEBUG, "byteclk: %ldM", ulrdbyteclk);
    ...
    LTLog(LOG_DEBUG, "ck_post (0xD4A9) = 0x%02x", pstMipiTxDPhy->ucClkPost);
    LTLog(LOG_DEBUG, "ck_zero (0xD4A7) = 0x%02x", pstMipiTxDPhy->ucClkZero);
    LTLog(LOG_DEBUG, "hs_lpx  (0xD4A4) = 0x%02x", pstMipiTxDPhy->ucHsLpx);
    LTLog(LOG_DEBUG, "hs_prep (0xD4A5) = 0x%02x", pstMipiTxDPhy->ucHsPrep);
    LTLog(LOG_DEBUG, "hs_trail(0xD4A6) = 0x%02x", pstMipiTxDPhy->ucHsTrail);
    LTLog(LOG_DEBUG, "hs_rqst (0xD48A) = 0x%02x", pstMipiTxDPhy->ucHsRqStPre);
}
